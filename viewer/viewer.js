...
// Helper functions for IE11 SVG class manipulation
function addClass(el, className) {
    if (el.classList) {
        ...
        if (currentClass.indexOf(className) === -1) {
            el.setAttribute('class', currentClass + (currentClass ? ' ' : '') + className);
        }
    }
}
...
function removeClass(el, className) {
    if (el.classList) {
        ...
        // IE11 SVG fallback
        var currentClass = el.getAttribute('class') || '';
        var newClass = currentClass.replace(new RegExp('(\\s|^)' + className + '(\\s|$)'), ' ').trim();
